...
const Doctor = ({ipfs, mediChain, account}) => {
  ...
  const submitDiagnosis = async (e) => {
    ...
    record.treatments = [
      { disease, treatment, charges, prescription: file, date: formattedDate, doctorEmail: doctor.email },
      ...record.treatments
    ]
      ...
      { doctor ?
        <>
          ...
          { patientRecord ? <Modal id="modal" size="lg" centered show={showRecordModal} onHide={handleCloseRecordModal}>
            <Modal.Header closeButton>
              <Modal.Title id="modalTitle">Record for: {patient.name}</Modal.Title>
            </Modal.Header>
            <Modal.Body>
                <Table striped bordered hover size="sm">
                  <thead>
                  ...
                  <tbody>
                    {
                      patientRecord.treatments.map((t, idx) => {
                        return (
                          <tr key={idx+1}>
                            <td>{t.disease}</td>
                            <td>{t.treatment}</td>
                            <td>{t.charges}</td>
                            <td>{ t.prescription ?
                              <a href={`https://ipfs.io/ipfs/${t.prescription}`} target="_blank" rel="noopener noreferrer">View</a>
                              : "No file attached"
                            }</td>
                            <td>{t.date}</td>
                            <td>{t.doctorEmail}</td>
        ...
        : <h3>You are not authorized to view this page <Link to='/login'>Login</Link></h3>
      }
    </div>
  )
}
