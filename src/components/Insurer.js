...
const Insurer = ({mediChain, account, ethValue}) => {
    ...
    const handleShowRecordModal = async (e, patient) => {
        var record = {}
        await fetch(`${process.env.REACT_APP_IPFS_API}/ipfs/${patient.record}`)
            .then(res => res.json())
                                            ...
                                            <tr key={idx+1}>
                                                ...
                                                <td>
                                                    { treatment.prescription ? 
                                                        <Link to={`${process.env.REACT_APP_IPFS_API}/ipfs/${treatment.prescription}`} target="_blank"><Button variant="coolColor">View</Button></Link>
