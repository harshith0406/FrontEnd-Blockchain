...
const Patient = ({ mediChain, account, ethValue }) => {
  ...
  const handleShowRecordModal = async () => {
    try {
      const response = await fetch(`${process.env.REACT_APP_IPFS_GATEWAY}/ipfs/${patient.record}`);
      if (!response.ok) {
        ...
        <>
          <div className='box'>
            ...
            <div>
              ...
              <Button variant='coolColor' style={{ width: '20%', height: '4vh' }} onClick={handleShowRecordModal}>
                View Records
              </Button>
            </div>
          </div>
                  ...
                  docList.map((doc, idx) => {
                    return (
                      <tr key={idx}>
                        ...
                        <td>{doc.email}</td>
                        <td>
                          <Button className='btn-danger' onClick={() => revokeAccess(doc.email)}>
                            Revoke
                          </Button>
                        </td>
                      </tr>
                  ...
                  transactionsList.map((transaction, idx) => {
                    return (
                      <tr key={idx + 1}>
                        ...
                        <td>
                          {transaction.settled ? (
                            <span className='badge rounded-pill bg-success'>Settled</span>
                          ) : (
                        ...
                        <td>
                          {!transaction.settled ? (
                            <Button className='btn-coolColor' onClick={(e) => settlePayment(e, transaction)}>
                              Pay
                            </Button>
                          ) : (
                            ...
                            <tr key={idx + 1}>
                              ...
                              <td>
                                {treatment.prescription ? (
                                  <Link to={`${process.env.REACT_APP_IPFS_GATEWAY}/ipfs/${treatment.prescription}`} target='_blank'>
